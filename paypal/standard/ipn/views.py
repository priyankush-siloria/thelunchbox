...
def ipn(request):
    ...
    if data is not None:
        ...
        for date_field in date_fields:
            if data.get(date_field) == 'N/A':
                del data[date_field]
            if date_field == "payment_date":
                del data[date_field]
                data[date_field] = timezone.localtime(timezone.now())
    ...
    if flag is not None:
        ...
        if request.is_secure() and 'secret' in request.GET:
            ...
            ipn_obj.verify()
    for field in ipn_obj._meta.get_fields():
        if hasattr(ipn_obj, field.name):
            if getattr(ipn_obj, field.name) == "b''":
                setattr(ipn_obj, field.name, "")
